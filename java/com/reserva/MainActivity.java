...
public class MainActivity extends Activity implements OnSeekBarChangeListener,
	...
	TextView cuantosninios;
	TextView cuantoshabitacion;
	TextView cuantosalberca;
	Button fecha, hora;
	SeekBar barraPersonas;
	SeekBar barraAlberca;
	SeekBar barraniños;
	SeekBar barrahabitacion;
	...
	String cuantosNiñosFormat = "";
	String cuantosHabitacionFormat = "";
	String cuantosAlbercaFormat = "";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cuantosninios = (TextView) findViewById(R.id.cuantosniños);
		barraniños = (SeekBar) findViewById(R.id.niños);
		barraniños.setOnSeekBarChangeListener(this);

		cuantoshabitacion = (TextView) findViewById(R.id.tipohabitacion);
		barrahabitacion = (SeekBar) findViewById(R.id.habitacion);
		barrahabitacion.setOnSeekBarChangeListener(this);

		cuantosalberca = (TextView) findViewById(R.id.cuantosalberca);
		barraAlberca  = (SeekBar) findViewById(R.id.alberca);
		barraAlberca.setOnSeekBarChangeListener(this);
		...
		cuantosNiñosFormat = cuantosninios.getText().toString();
		// cuantasPersonasFormat = "personas: %d";
		cuantosninios.setText("Niños: 1"); // condicion inicial

		cuantosHabitacionFormat = cuantoshabitacion.getText().toString();
		// cuantasPersonasFormat = "personas: %d";
		cuantoshabitacion.setText("Tipo de Habitación: 1"); // condicion inicial

		cuantosAlbercaFormat = cuantosalberca.getText().toString();
		// cuantasPersonasFormat = "personas: %d";
		cuantosalberca.setText("Con alberca(0 = no, 1 = si): 0"); // condicion inicial
	...
	public void onProgressChanged(SeekBar barra, int progreso,
		...
		numNiños = String.format(cuantosNiñosFormat,
				barraniños.getProgress() + 1);
		niños = barraniños.getProgress() + 1; // este es el valor que se
		// guardara en la BD
		// Si no se mueve la barra, enviamos el valor personas = 1
		cuantosninios.setText(numNiños);

		numhabitacion = String.format(cuantosHabitacionFormat,
				barrahabitacion.getProgress() + 1);
		habitacion = barrahabitacion.getProgress() + 1; // este es el valor que se
		// guardara en la BD
		// Si no se mueve la barra, enviamos el valor personas = 1
		cuantoshabitacion.setText(numhabitacion);

		numalberca = String.format(cuantosAlbercaFormat,
				barraAlberca.getProgress() + 1);
		alberca = barraAlberca.getProgress() + 1; // este es el valor que se
		// guardara en la BD
		// Si no se mueve la barra, enviamos el valor personas = 1
		cuantosalberca.setText(numalberca);
	}
	...
	public void reserva(View t) {
		...
		datos.putInt("niños", niños);
		datos.putInt("habitacion", habitacion);
		datos.putInt("alberca", alberca);
